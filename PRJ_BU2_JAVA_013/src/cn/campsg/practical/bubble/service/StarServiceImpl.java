...
public class StarServiceImpl implements StarService {
	...
	private ArrayList<Integer> getNullColumns(StarList currentStarList){
		...
		for(int i = 0;i < MAX_COLUMN_SIZE;i++){
				star = currentStarList.lookup(MAX_COLUMN_SIZE - 1,i);
				if (star == null) {
					nullList.add(i);
				}
		}
	...
	public StarList getXMovedStars(StarList currentStarList) {
		...
		ArrayList<Integer> nullList = getNullColumns(currentStarList);
		if (nullList == null) {
			return null;
		}
		StarList toBeMovedStarList = new StarList();
		int moveDistance = 0;
		MovedStar movedStar = null;
		for(int i = nullList.get(0);i < MAX_COLUMN_SIZE;i++){
			//空列
			if (nullList.contains(i)) {
				...
				for(int j = MAX_ROW_SIZE - 1; j >= 0; j--){
					...
					if (star == null) {
						...
						movedStar.setMovedPosition(new Position(movedStar.getPosition().getRow(),movedStar.getPosition().getColumn() - moveDistance));
						toBeMovedStarList.add(movedStar);
					}
				}
			}
		}

...
public class StarServiceImpl implements StarService {
	...
	public StarList getYMovedStars(StarList clearStars,
		...
		StarList toBeMovedStars = new StarList();
		int column = 0;
		int willMoveRowCount = 0;
		//获取待消除泡泡糖集合的分组对象
		Map<Integer , StarList> map = StarsUtil.group(clearStars);
		
		//获取group返回的【Map】对象中Key的迭代器【Iterator】
		Iterator iterator = map.keySet().iterator();
		
		while(iterator.hasNext()){
			willMoveRowCount = 0;
			//将代表当前列的变量初始值，修改为迭代器中动态获取的key值。
			column = (int)iterator.next();
			//通过2-5.1获取的key值，获取Map中对应的value（StarList），此集合为当前列的待消除的泡泡糖集合。
			clearStars = map.get(column);
			//将判断“待移动泡泡糖”的起始位置变量修改为：循环获取的每一列“待清除泡泡糖”集合的最后一个元素的行值。
			Star lastStar = clearStars.get(clearStars.size() - 1);
			
			for(int i = lastStar.getPosition().getRow();i >= 0 ;i--){
				Star star = currentStarList.lookup(i, column);
				if (star == null) {
					break;
				...
				MovedStar movedStar = new MovedStar(star.getPosition(),star.getType(),new Position(i + willMoveRowCount,column));
				toBeMovedStars.add(movedStar);
			}
		}

		
		
		return toBeMovedStars;
